...
`design-audit/baseline/inventory.json`
— the component inventory: the existing library to reuse (do not re-create).
`design-audit/config.json` — the canonical Figma
file keys (source / target / benchmark) and library key.
`docs/responder-app-flow-map.md` — the full
...
`design-audit/baseline/inventory.json` and Figma `34:995` — is there
already a component (button, chip, badge, tabs, input, avatar, card, row…)?
The AOH ShadCN source (`IQxwVUqzckYn7u7nNhX6lF`) — bring it FROM there.
...
Before confirming any design is "done," run the Component Library Verification
checklist in `DESIGN.md`. Do not
declare a design complete until every element resolves to an existing library
component (Figma `34:995` / `inventory.json`) or is flagged as a documented gap.
Git
