...
## Inputs
- Read `design-audit/config.json` for the Figma file key, watched node, the
  AOH UI/ShadCN library (single source of truth), and the `mobile` block (M3
  reference file + mobile rules spec).
- Read `design-audit/baseline/inventory.json` — the last known component snapshot.
- Read `design-audit/mobile-rules.md` — the M3-benchmarked thresholds mobile
  components must meet (touch targets, spacing, type, safe zones, grid, radius, states).
...
4. **Check AOH UI/ShadCN adherence:**
   ...
   - **Raw-colour check (P2)** per `design-audit/AOH-ADHERENCE.md`: via `get_design_context`,
     flag any fill/stroke/text using **raw hex/rgba** instead of an AOH `color-*`/`severity/*`
     variable, and any text not using a `text-*/font-*` style. (Known offender: the Comms/PTV suite.)
...
5. **Check mobile ergonomics vs Material 3 thresholds** (per `mobile-rules.md`):
   - Scope = the mobile components (watched node / "Mobile Components" page).
   - From `get_metadata`, flag interactive components/instances with height OR
     width **< 48 dp** (touch target). Tabs, buttons, icon buttons, list rows,
     chips, switches, checkboxes count.
   - From `get_variable_defs` / `get_design_context`, flag **body/content text
     < 14 sp** (< 12 sp = P1), paddings/gaps **not a multiple of 4** or not bound
     to an AOH `Spacing/*` token, and radii not bound to an AOH `Radius/*` token.
   - Flag full-screen frames missing **safe-area insets** (status bar / nav bar).
   - Spot-check 4-col / 16 dp **grid margins** on a sample of screens.
   - Cross-reference the M3 reference file (`m3ReferenceFileKey` in config) only
     to confirm a threshold when unsure. Severity per the mapping in `mobile-rules.md`.
6. **Write the report** to `design-audit/reports/<TODAY>.md` using the template in
   `design-audit/reports/_TEMPLATE.md`. Sections: New, Duplicates, Flags,
   Adherence, **Mobile spec (M3) compliance**, What-can-change, Open questions.
   Every finding cites the node id/name + the measured value as evidence. Separate
   objective issues from opinion.
7. **Update the baseline:** overwrite `design-audit/baseline/inventory.json` with
   the fresh snapshot (and updated `localTokens`) so tomorrow's run diffs correctly.
8. **Commit & push** report + updated baseline to branch
   `claude/intelligent-lovelace-wnqhrw` with message
   `design-audit: <TODAY> report`. Do **not** open a PR.
9. **Post the summary in chat** (counts of new/removed/duplicates/flags + mobile
   violations + the report path). Keep it short; the file has the detail.
